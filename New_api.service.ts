...
export class ApiService {
  ...
  private baseURLArr:string[] = ['http://srv12webpta:8040/api/', 'http://srv12webdev:8040/api/', 'http://localhost:5200/api/', 'http://localhost:8002/api/'];
  private baseUrl = 'http://srv12webdev:8040/api/';

  public setBaseUrl(idx:number){
    this.baseUrl = "";
    if (idx < this.baseURLArr.length)
    {
      this.baseUrl = this.baseURLArr[idx];
    }
  }
  ...
  public get(api: string, payload?: any): Observable<Response> {
    ...
    const options = new RequestOptions({
      ...
      headers: new Headers({ 'Content-Type': 'application/json','Access-Control-Allow-Origin':'*','Access-Control-Allow-Credentials':true,'Access-Control-Allow-Methods':'HEAD, GET, POST, PUT, PATCH, DELETE','Access-Control-Allow-Headers': 'Origin, Content-Type, X-Auth-Token'}),
      params: payload
    });
    this.processCount++;
  ...
  public post(api: string, payload: any, payloadBody?: any): Observable<Response> {
    ...
    this.processCount++;
    return this.http.post(this.buildUrl(api),payload,options).pipe(map(data => this.extractData(data)));
  }
  ...
  private buildUrl(controller: string): string {
    ...
    let url:string = this.baseUrl;//'http://srv12webdev:8040/api/'   //'http://localhost:5200/api/';  //'http://srv12webdev:8020/LiveTestTyrusServices';
